...
EnableMaintenanceMode () {
    ...
    sudo -u ${webServerUser} php ${ncServerDir}/occ maintenance:mode --on
    echo "Done"
    echo
}
DisableMaintenanceMode () {
    echo "Switching off maintenance mode..."
    sudo -u ${webServerUser} php ${ncServerDir}/occ maintenance:mode --off
    echo "Done"
    echo
}
backup_server ()
{
    ...
    tar -cpzf ${ncServerTempDir}/nc_server_${DATE}.tar.gz -C ${ncServerDir} .
    cp ${ncServerTempDir}/nc_server_${DATE}.tar.gz ${ncServerBackupDir}/
    #ToDo: add scp copy to backup server
    #scp -p ${ncServer}:${ncServerTempDir}/nc_server_${DATE}.tar.gz ${ncServerBackupDir}/
    sleep 1
    rm -f ${ncServerTempDir}/nc_server_${DATE}.tar.gz
...
backup_data ()
{
    ...
    #ToDo: add rsync copy to backup server
    #rsync -axv --delete --link-dest=${ncDataFolderBackup}/Latest ${ncServer}:${ncServerData} ${ncDataFolderBackup}/Processing-${DATE}
    rsync -axv --delete --link-dest=${ncDataFolderBackup}/Latest ${ncServerData} ${ncDataFolderBackup}/Processing-${DATE}
    sleep 1
...
backup_db ()
{
    ...
    sleep 1
    if [[ ! -e ${ncServerTempDir} ]]; then mkdir -p ${ncServerTempDir}; fi
    mysqldump --single-transaction -h localhost -u${dbUser} -p${dbPass} ${dbName} > ${ncServerTempDir}/nc_db_${DATE}.sql
    ...
    cp ${ncServerTempDir}/nc_db_${DATE}.sql ${ncDbFolderBackup}/nc_db_${DATE}.sql
    echo "Backup database is done"
    sleep 1
    rm -r ${ncServerTempDir}/nc_db_${DATE}.sql
}
